...
func shoot_at_plane(weapon: Node3D):
	...
	if player:
		...
		if projectile_type == GameData.WEAPON.CANON or projectile_type == GameData.WEAPON.ARROW:
			...
			var future_position = get_refined_future_position(weapon, player_position, player_velocity, projectile.speed)
			var direction = (future_position - weapon.global_transform.origin).normalized()
			
			get_tree().root.add_child(projectile)
			projectile.global_transform.origin = weapon.global_transform.origin
...
func get_refined_future_position(weapon, player_position, player_velocity, projectile_speed):
	""""""
	
	var initial_prediction = get_future_player_position(weapon, player_position, player_velocity, projectile_speed)
...
func get_future_player_position(
	...
	var distance_to_player = player_position.distance_to(weapon.global_transform.origin)
